...
Setelah itu langkah selanjutnya adalah membuat _Forward zone_ untuk domain <code>{FQDN}</code> dengan menyalin berkas `/etc/bind/db.local` ke <code>{BIND_F_ZONE_FILE_PATH}</code> dan memodifikasi nya seperti baris kode berikut.
