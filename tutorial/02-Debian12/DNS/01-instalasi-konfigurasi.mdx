...
Pada contoh konfigurasi di atas ada beberapa tipe _record_ yang ditambahkan yaitu `A` untuk domain dengan _ipv4_, `NS` (**_Name Server_**) dan `MX` (**_Mail Exchanger_**). Langkah selanjutnya adalah membuat _Reverse zone_ untuk domain <code>{v.FQDN}</code> dengan menyalin file `/etc/bind/db.127` ke {v.BIND_R_ZONE_FILE_PATH}.
